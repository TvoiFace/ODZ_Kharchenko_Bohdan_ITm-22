...
class Filters {
    ...
    fun getSearchDescription(context: Context): String {
        ...
        if (price > 0) {
            desc.append(" for ")
            desc.append("<b>")
            desc.append(HotelUtil.getPriceString(price))
            desc.append("</b>")
        }
    ...
    fun getOrderDescription(context: Context): String {
        return when (sortBy) {
            Hotel.FIELD_PRICE -> context.getString(R.string.sorted_by_price)
            Hotel.FIELD_POPULARITY -> context.getString(R.string.sorted_by_popularity)
    ...
    companion object {
        ...
        val default: Filters
            get() {
                val filters = Filters()
                filters.sortBy = Hotel.FIELD_AVG_RATING
